...
The current cleanup correction keeps the private experiment disabled. One shared loader-only profile comparator is used by normal plugin removal and manual recovery; it permits only changed content in the same recognized BepInEx log path and retains exact complete-baseline verification after loader rollback. The PowerShell wrapper now offers `-Mode Rollback` for the supported C# rollback operation, derives the repository HEAD commit, and passes the explicitly supplied executable-relative path so C# can bind it to discovered evidence. Head `2139d41812b5da09c620a1685a217de1caa3510e` passed hosted run `31025440280` on both runners with 13 TRX files and 350 tests per runner under SDK `10.0.100`; no private run was performed. The branch is ready for one separately authorized final private verification.
