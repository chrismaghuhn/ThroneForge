...
Task-6 deployment consumes the shared `LoaderSmokeTestStatePaths` contract. The canonical persisted files are `baseline-copy-manifest.json` and `transaction-state.json`; deployment rejects the legacy `baseline.json` name. Loader transaction validation is bound to the saved disposable baseline manifest, not the original-installation manifest. State-gate failures are surfaced as stable sanitized categories without paths, manifests, or inner exception text.

The runtime marker reads API and Contracts identities from the actual loaded assemblies. The earlier private report did not contain that measurement, and the subsequent `final15` attempt did not reach plugin launch because deployment-state validation failed before any plugin file was written. The corrected fresh run passed and recorded actual runtime identities, one nonce marker, exact package recapture/admission, plugin removal, loader rollback, and complete original/disposable post-verification. No plugin, loader, game binary, raw log, private manifest, or experiment state is committed.
- Validation outcome: pre-private hosted synthetic CI passed in run `30998768487`, and final hosted CI passed in run `30999995802` for documentation head `fb8bcf46cafe1afa82ca3044b94a85251e704529` on Windows and Ubuntu with SDK `10.0.100`, 13 TRX files, and 279 tests per runner. One corrected private run then passed; M1 Task 7 was later started separately and its private lifecycle run failed before deployment.
