...
The active branch `agent/m1-task7-loader-launch-investigation` is repository-only. It adds bounded evidence for the `LoaderLaunch` boundary and sanitized relative manifest-drift evidence for future recovery diagnostics. The final authorized private experiment remains immutable `Failed` at `LoaderLaunch`; its historical raw loader log and exact drift file were not retained, so no exact historical root cause is claimed. The old external experiment root is untrusted and unused. No private lifecycle or recovery execution is authorized, and M1 Task 8 remains blocked. Implementation commit `c9429c5d56d11531e4c5bfbbf63a793d6d927edc` passed hosted run `31045763385`; final documentation/test head `142b65fa01e201917b379828a6c4c6ee4dfc2965` passed hosted run `31046374898` on Windows and Ubuntu with SDK `10.0.100`, 13 TRX files and 372 passing tests per runner. All failure, error, timeout, abort, inconclusive, not-executed and skipped counters were zero.

M1 - Task 7 public Unity lifecycle binding and synthetic lifecycle smoke test remains incomplete on `agent/m1-lifecycle-binding-smoke-test` from merged `main@9c90657f35406b353b495f0889e1cacf571668e0`. The repository correction passed Hosted CI, and the one authorized final private run is recorded as `Failed` at `LoaderLaunch` with `loader-launch-failed`; no lifecycle binding conclusion is claimed and no retry is authorized. The single permitted recovery-only rollback was then executed after green hosted CI and failed before loader mutation with `recovery-runtime-drift`. The C# `LifecycleExperimentOrchestrator` owns the real CLI operation, typed stage evidence, immutable primary failure state, cleanup/postchecks, disposable runtime/readiness/indicator checks, and report generation. PowerShell is only an explicit-input wrapper. Recovery did not complete loader rollback; the original-installation postcheck from the experiment remains passed. The two earlier private attempts remain historical `Failed` evidence.

The earlier orchestration correction head `dbbcc3a` passed hosted run `31012843103` with 327 tests per runner; the current ownership correction supersedes it and is validated by run `31016602789`. The final sanitized report head `e0bfd3958d4f2add190877b436737f7a0946b6e9` passed final hosted run `31028231111` with 350 tests per runner; no further private run was performed.

PR #7 review correction is repository-only: deployed-plugin recovery removes the exact ownership-bound plugin before rollback-drift validation and preserves the cleanup result if a later phase fails. BaselineLaunch and LoaderLaunch now propagate typed manual-closure/process-active evidence, so active profiles never enter cleanup. No private lifecycle or recovery execution was performed for this correction.

Correction head `1cd903c0aa1942d793409893d458939660e40c9d` passed hosted run `31039015040` on Windows and Ubuntu with SDK `10.0.100`, 13 TRX files and 360 tests per runner. All counters were zero for failures, errors, timeouts, aborts, inconclusive, not-executed and skipped tests; no TRX overwrite warning occurred.

The follow-up review correction keeps `PluginRemovalStatus=NotRequired` in every later recovery failure when no plugin was deployed. BaselineLaunch manual closure now persists `NotApplied`, exposes `no-loader-cleanup-required`, and emits no executable loader-rollback command. No private lifecycle or recovery execution was performed.

Correction head `bb960efd9e0715639807469a109b3ce28f700c72` passed PR run `31041425981` and push run `31041429428` on Windows and Ubuntu with SDK `10.0.100`. Each runner produced 13 TRX files and 362 passing tests; all failure, error, timeout, abort, inconclusive, not-executed and skipped counters were zero, with no TRX overwrite warning.

This production-path correction adds Task-6 ownership creation before preparation, an owned-root source package builder, side-effect-preserving evidence application, typed manual-closure recovery and C# rollback, post-verification-time loader-only manifest capture, profile-bound package/manifest/Unity paths, and runtime API/Contracts identities derived from captured package metadata. The current cleanup correction adds a shared volatile-log comparison for normal and recovery cleanup, an executable C# rollback wrapper mode, discovered-executable binding, and repository-HEAD commit binding. Implementation head `2139d41812b5da09c620a1685a217de1caa3510e` passed hosted run `31025440280`; final sanitized report head `e0bfd3958d4f2add190877b436737f7a0946b6e9` passed hosted run `31028231111` on Windows and Ubuntu with SDK `10.0.100`, 13 TRX files and 350 tests per runner. The recovery hardening head `1549552810e826afc415355f87742faaecafc354` passed hosted run `31032572956` with 355 tests per runner. Every test passed, every TRX artifact was parsed, and no TRX overwrite warning occurred. The authorized private run failed once at `LoaderLaunch` with `loader-launch-failed`; its one recovery-only rollback returned `recovery-runtime-drift` before loader mutation, and no retry is authorized.
...
- Task 6 is complete for this bounded synthetic-plugin experiment. It does not establish Thronefall API, Harmony, lifecycle, catalog, wave, or production plugin compatibility. M1 Task 7 was subsequently started on its own branch; its private lifecycle run is recorded separately as failed before deployment.

## Next task

The next task was a separately scoped M1 Task 7 plan and review. Do not infer game API/lifecycle behavior from the synthetic-plugin result; the Task-7 private run did not establish a lifecycle binding.
