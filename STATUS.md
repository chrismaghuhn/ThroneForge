...
M1 - Task 7 public Unity lifecycle binding and synthetic lifecycle smoke test remains incomplete on `agent/m1-lifecycle-binding-smoke-test` from merged `main@9c90657f35406b353b495f0889e1cacf571668e0`. This correction is repository-only and does not authorize another private run. The existing hosted baseline `31009835130` passed with 13 TRX files and 313 tests per runner using SDK `10.0.100`. The one permitted corrective private run stopped at `OriginalPreflight` with stable category `original-preflight-failed` because the harness did not parse the discovery CLI's selected-executable evidence; no loader transaction, package, deployment or lifecycle marker was produced.

Task-7 orchestration correction head `dbbcc3a` passed hosted run `31012843103`: Windows and Ubuntu each uploaded 13 TRX files representing 327 tests with zero failures, errors, skips or not-executed tests under SDK `10.0.100`. No private run was performed in this correction.

## State

Task 6 was merged through PR #6. Task 7 verifies only the public `UnityEngine.Application.quitting` source with binding ID `unity-application-quitting-v1`. It binds BepInEx `Awake` to one synthetic mod initialization and the public quitting event to one shutdown of the same mod. No Harmony hook, game-defined type, gameplay state, catalog, save data, or custom wave is in scope. The lifecycle experiment is synchronous-only; `OnDestroy` is cleanup fallback and cannot satisfy the pass criteria. The correction persists sanitized stage/failure state, shares the tested host with the generated plugin, consumes only machine-readable runtime evidence, and independently records plugin removal, loader rollback, disposable restoration and original postchecks. The prior private run remains failed before loader transaction persistence.
