...
Task 6 keeps private experiments separate from hosted CI. The harness now requires a fingerprint-bound ownership record, derives loader/profile readiness, recaptures and admits the exact three package files immediately before transactional deployment, validates package metadata, and records actual runtime API/Contracts identities. The final private attempt remains recorded as `Failed` because a Task-3/Task-6 baseline-state filename mismatch stopped deployment before plugin files were written; rollback and original post-checks passed. The correction centralizes the canonical state paths and binds transaction validation to the disposable baseline. Corrected hosted CI and one fresh private rerun are still pending. No plugin pass is currently claimed, and M1 Task 7 has not started. No plugin, loader, game binary, raw log, archive, nonce, or private path is committed. See the [Task-6 report](docs/discovery/1ddd8982e790969cb208cf91bb1489123413d167f9e07cd0416ab6739d4fcd7d-synthetic-plugin-smoke-test.md), [Task-6 design](docs/superpowers/specs/2026-08-05-m1-disposable-bepinex-plugin-smoke-test-design.md), and [discovery safety guide](docs/discovery/README.md).
