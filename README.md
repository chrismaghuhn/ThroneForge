...
This repository contains the completed M0 architecture skeleton, M1 discovery tasks 1 and 2, and the merged Task 3 loader-bootstrap smoke test plus its fail-closed hardening. M1 Task 4 has started on a separate branch with only a portable plugin/runtime admission boundary in scope. The historical private bootstrap result is retained with an explicit limitation: complete pre/post manifests were not retained. It is not a functioning Thronefall mod: no plugin is loaded, and no game-facing runtime, catalog exporter, lifecycle binding, or custom-wave implementation exists.
