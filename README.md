...
Task 6 is complete and merged. M1 Task 7 remains incomplete on `agent/m1-lifecycle-binding-smoke-test` and is limited to the public Unity `Application.quitting` lifecycle binding. Correction head `bdd871fa263d5b97fc4a20160ee110d32f406c99` passed hosted run `31008620029`, but the one permitted corrective private run stopped at `OriginalPreflight` with `original-preflight-failed` because the harness did not parse the discovery CLI's selected-executable output. No loader transaction, package deployment or lifecycle evidence was produced, and no further private run was performed. The correction includes stage-specific diagnostics, a shared tested lifecycle host, strict prefixed-marker parsing, and independent rollback evidence. The task does not inspect Thronefall internals, use Harmony, access gameplay state, or implement catalog/custom-wave behavior.
...
M0 architecture, hardening, and the final TRX artifact-preservation fix passed hosted Windows/Linux CI. M1 discovery tasks 1 and 2 are complete on protected `main`; PR #2 merged at `d3f1bb4fde9f77efbb84349f440385cc89002c86`. M1 Task 3 and its hardening merged by PR #3 at `06554d8`; the historical private bootstrap result retains its complete-manifest limitation. M1 Task 4 and its trust-evidence hardening merged by PR #4 at `5f4b4dd0714d0cffaf9f3267b6f0651ecf6e043e`. M1 Task 5 is merged and hosted-verified. M1 Task 6 is merged and hosted-verified with one corrected private synthetic-plugin pass. M1 Task 7 is implemented and hosted-verified, but its one permitted private lifecycle run failed before package deployment; the public Unity binding remains unverified. No game API, Harmony, catalog exporter, or custom-wave functionality is claimed.
