...
This repository contains the completed M0 architecture skeleton, M1 discovery tasks 1 and 2, the merged Task 3 loader-bootstrap smoke test and hardening, the merged Task 4 portable plugin/runtime admission boundary, and the merged Task 5 repository-only synthetic plugin-load probe. M1 Task 6 is in progress: it prepares one fingerprint-bound, reversible BepInEx synthetic-plugin smoke test, but no private run has started. The repository remains a game-free architecture and test surface; it is not a functioning Thronefall mod and has no game-facing runtime, catalog exporter, lifecycle binding, or custom-wave implementation.

Task 6 deliberately keeps the private experiment separate from hosted CI. The source-only plugin is built only after bounded local BepInEx/Unity/API/Contracts metadata selects `netstandard2.0` or `netstandard2.1`; the exact three-file package is then bound to the fixed game fingerprint and approved immediately before deployment. No plugin, loader, game binary, raw log, archive, nonce, or private path is committed. See the [Task-6 design](docs/superpowers/specs/2026-08-05-m1-disposable-bepinex-plugin-smoke-test-design.md) and [discovery safety guide](docs/discovery/README.md).
