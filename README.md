...
This repository contains the completed M0 architecture skeleton, M1 discovery task 1, and M1 task 2 runtime-compatibility hardening. M1 task 3 has a private `Passed` bootstrap result for the documented fingerprint, with hosted synthetic CI verification still pending. It is not a functioning Thronefall mod: no plugin, game-facing runtime, catalog exporter, lifecycle binding, or custom-wave implementation exists.
...
M0 architecture, hardening, and the final TRX artifact-preservation fix passed hosted Windows/Linux CI. M1 discovery task 1 and task 2 are complete on protected `main`; PR #2 merged at `d3f1bb4fde9f77efbb84349f440385cc89002c86`. Main run [30853440786](https://github.com/chrismaghuhn/ThroneForge/actions/runs/30853440786) passed with 10 TRX files and 99 tests per runner using SDK `10.0.100`. M1 task 3 privately passed the reversible BepInEx bootstrap check for the documented fingerprint; this branch still requires hosted synthetic CI verification. `main` remains the protected default branch. ThroneForge remains an architecture skeleton, not a functioning Thronefall mod; discovery must document evidence without guessing game internals.
