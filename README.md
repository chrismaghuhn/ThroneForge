...
Task 6 is complete and merged. M1 Task 7 is implemented on `agent/m1-lifecycle-binding-smoke-test` and is limited to the public Unity `Application.quitting` lifecycle binding. Hosted synthetic CI passed, but the one permitted fresh private run failed before a loader transaction/package deployment, so no lifecycle binding conclusion is claimed. The task does not inspect Thronefall internals, use Harmony, access gameplay state, or implement catalog/custom-wave behavior.
