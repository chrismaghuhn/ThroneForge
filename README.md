...
Task 6 is complete and merged. M1 Task 7 is being hardened on `agent/m1-lifecycle-binding-smoke-test` and remains limited to the public Unity `Application.quitting` lifecycle binding. The reviewed hosted baseline passed, but the one permitted fresh private run failed before a loader transaction/package deployment, so no lifecycle binding conclusion is claimed. The correction adds stage-specific sanitized diagnostics, a shared tested lifecycle host, strict prefixed-marker parsing, and independent rollback evidence. The task does not inspect Thronefall internals, use Harmony, access gameplay state, or implement catalog/custom-wave behavior.
